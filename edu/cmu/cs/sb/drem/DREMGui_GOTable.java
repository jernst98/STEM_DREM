...
public class DREMGui_GOTable extends JPanel implements ActionListener {
	 ...
	 */
	public DREMGui_GOTable(final DREMGui theDREMGui, JFrame theFrame,
			DREM_DataSet theDataSet, double[][] bindingpval,
			int[][] bindingpvalindex, double[][] bindingpvalTF,
			int[][] bindingpvalTFindex, String[] tfNames,
		...
		if (tgr.goRecArray.length == 0) {
			...
			for (int nindex = 0; nindex < tgr.goRecArray.length; nindex++) {
				...
				if (tgr.goRecArray[nindex].dcorrectedpvalue < 0.001) {
					if (theDataSet.tga.brandomgoval) {
						...
						tabledata[nindex][7] = Util
								.doubleToSz(tgr.goRecArray[nindex].dcorrectedpvalue);
					}
				} else {
				...
				tabledata[nindex][8] = ""
						+ nf1.format(tgr.goRecArray[nindex].dcategoryselect
								/ dexpected);
			}
			...
			final double[][] fbindingpval = bindingpval;
			final int[][] fbindingpvalIndex = bindingpvalindex;
			final double[][] fbindingpvalTF = bindingpvalTF;
						...
						javax.swing.SwingUtilities.invokeLater(new Runnable() {
							public void run() {
								...
								DREMGui_GOGeneTable newContentPane = new DREMGui_GOGeneTable(
										theDREMGui, frame, finaltheDataSet,
										theDREMGui.theTimeiohmm, fbindingpval,
										fbindingpvalIndex, fbindingpvalTF,
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (szCommand.equals("save")) {
			...
			szMessage += "\nThe table has nine columns:"
					...
					+ tgr.dselect
					+ " and "
					+ "for the data  #Total_Unique_Genes_on_Array="
					+ ((int) tgr.dall)
					+ ".\n"
